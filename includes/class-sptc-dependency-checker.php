 ...
 * @since 2.0.0
 */
class Sensei_Post_To_Course_Dependency_Checker {
	...
	public static function deactivate_self() {
		deactivate_plugins( SENSEI_POST_TO_COURSE_PLUGIN_BASENAME );
	}
	...
	public static function add_php_notice() {
		...
		$message        = sprintf(
			/* translators: %1$s is Minimum required PHP version; %2$s is Current PHP version. */
		...
		printf(
			...
			esc_html__( 'Learn more about updating PHP', 'sensei-post-to-course' ),
			/* translators: accessibility text */
			esc_html__( '(opens in a new tab)', 'sensei-post-to-course' )
		);
		echo '</p></div>';
	}
	...
	public static function add_sensei_notice() {
		...
		$message = sprintf(
			/* translators: %1$s is the minimum required version of Sensei LMS. */
